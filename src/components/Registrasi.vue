...
function validate() {
  if (!name.value.trim()) return 'Nama wajib diisi'
...
async function handleSubmit() {
  ...
  if (validationError) {
    ...
    passwordMismatch.value = validationError === 'Konfirmasi password tidak sama'
    return
  }
  ...
  try {
    ...
    errorMsg.value =
      e.message === '__CONFIRM_EMAIL__'
        ? 'Registrasi berhasil, cek email kamu untuk konfirmasi sebelum masuk.'
        : e.message
  } finally {
    loading.value = false
  }
    ...
    <div class="relative hidden overflow-hidden bg-[#0F172A] lg:flex lg:flex-col lg:justify-between lg:p-12">
      ...
      <div class="relative z-10 max-w-sm">
        <h2 class="text-3xl font-bold leading-tight text-white">
          Mulai kendalikan tagihanmu.
        </h2>
        <p class="mt-3 text-sm leading-relaxed text-white/60">
          Daftar dan dapatkan analisis, simulasi, dan rekomendasi penghematan listrik & air yang sesuai kebiasaanmu.
        </p>
    ...
    <div class="flex items-center justify-center bg-[#F8FAFC] px-4 py-12 sm:px-6 lg:px-16">
      <div class="w-full max-w-sm animate-[fadeInUp_0.5s_ease-out_forwards]">
        ...
        <p class="mt-2 text-sm leading-relaxed text-[#64748B]">
          Mulai analisis penggunaan listrik dan airmu dalam beberapa langkah.
        </p>
        ...
        <form class="flex flex-col gap-4" @submit.prevent="handleSubmit">
          <div>
            <label for="name" class="text-sm font-medium text-[#0F172A]">Nama</label>
            <input
              id="name"
              v-model="name"
              type="text"
          ...
          <div>
            <label for="confirm-password" class="text-sm font-medium text-[#0F172A]">
              Konfirmasi Password
            </label>
            <div class="relative mt-1.5">
              <input
                ...
                placeholder="••••••••"
                class="w-full rounded-xl border bg-white px-4 py-2.5 pr-11 text-sm text-[#0F172A] placeholder:text-slate-400 transition-shadow focus:outline-none focus:ring-4"
                :class="passwordMismatch
                  ? 'border-red-300 focus:border-red-400 focus:ring-red-100'
                  : 'border-slate-200 focus:border-[#16A34A] focus:ring-[#16A34A]/10'"
              />
              <button
                type="button"
            ...
            <p v-if="passwordMismatch" class="mt-1.5 text-xs text-red-500">
              Password tidak sama, coba periksa lagi.
            </p>
          </div>
